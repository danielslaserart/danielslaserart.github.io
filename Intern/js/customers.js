import { $, esc, euro, num, uid } from "./utils.js?v=6.4.4";
import { state, save, flushCloudSave, getRealProjects, normalizeCustomerRecord } from "./storage.js?v=6.4.4";
import { appAlert, appConfirm, appForm } from "./dialogs.js?v=6.4.4";
...
function renderList(){
  ...
  box.innerHTML=rows.length?rows.map(c=>{const stats=priceStats(c),warning=activeWarnings(c).length;return `<article class="customer-row card"><button type="button" class="customer-row-open" data-customer-id="${esc(c.id)}"><div class="customer-row-main"><strong>${esc(c.companyName)}</strong><small>${esc(c.contactPerson||"Kein Ansprechpartner")}</small><span class="customer-list-alerts">${c.blocked?'<b class="customer-list-blocked">⛔ Gesperrt</b>':""}${warning?`<b class="customer-list-warning">⚠ ${warning} Warnhinweis${warning===1?"":"e"}</b>`:'<span class="customer-list-clear">✓ Keine Warnungen</span>'}</span></div>${stars(c.rating)}<span>${c.regular?"Ja":"–"}</span><span>${c.vip?"Ja":"–"}</span><span class="customer-warning-symbol">${warning||c.blocked?"⚠️":"–"}</span><b>${stats.lastAgreed!=null?euro(stats.lastAgreed):"–"}</b></button><button type="button" class="customer-project-count" data-customer-projects="${esc(c.id)}" title="Zugehörige Projekte öffnen">${stats.projects.length}</button></article>`;}).join(""):`<div class="empty-state">Keine passende Kundenakte.</div>`;

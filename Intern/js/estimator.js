import { $, num, euro, esc, uid } from "./utils.js?v=6.6.36";
import { state, save } from "./storage.js?v=6.6.36";
import { materialSelections, resolveMaterialSelection } from "./materials.js?v=6.6.36";
import { rounded, computePriceBreakdown, computePriceRecommendations } from "./calculator.js?v=6.6.36";
import { getPriceLadderData, renderPriceLadder } from "./price-ladder.js?v=6.6.36";
import { findSimilarProjects, learnedTimeFactor, learnedPriceSuggestion, saveLearningRecord } from "./learning.js?v=6.6.36";
import { appAlert, appForm, appConfirm } from "./dialogs.js?v=6.6.36";
import { renderMotifProfiles } from "./processing-profiles.js?v=6.6.36";
...
export function calculateMotifEstimator(){
  ...
  const customerBreakdown=customerPricing?computePriceBreakdown({
    orderType,machine:machineCost,work:workCost,extra,
  ...
  const recommendationParts=customerPricing?{
    ...
    difficultyPercent:customerPricing.difficultyPercent,risk:customerPricing.risk,express:customerPricing.express,
    profitPercent:num($('mcProfit').value),roundFn:rounded
  }:{orderType:"own",material,machine:machineCost,work:workCost,extra,overheadPercent:num(state.settings.overhead),reservePercent:num($('mcReserve').value),profitPercent:num($('mcProfit').value),roundFn:rounded};
